      ...
      <div class="card-body">
        <table id="example1" class="table table-bordered table-striped">
          ...
          <tbody>
            ...
            {{-- @php    $user= $item->id;
            $role_id= $user->user_role->first()->role_id; @endphp --}}
          <tr>
            <td>{{$item->name}}</td>
            <td>{{$item->user_name}}</td>
            <td>{{$item->email}} </td>
            <td></td>
            <td><img src="{{ asset('storage/user/'.$item->profile_photo_path) }}" height="50px" width="50px" alt="" title=""></td>
            <td>  
              <div class="row"> 
               <a href="{{ URL::to('users/' . $item->id . '/edit')}}" class="">
