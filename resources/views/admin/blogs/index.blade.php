    ...
    <div class="card-header">
      ...
      <div class="card-tools">
        @if(auth()->user()->permission('add blogs')) 
         <a href="{{route('blogs.create')}}"> <button type="button" class="btn btn-primary " >
              Create Blog
            </button></a>
            @endif
    ...
    <div class="card-body">
      <table id="example1" class="table table-bordered table-striped">
        ...
        <tr>
          ...
          <!--<td><button   class="blog_id btn btn-success me-md-2 " data-toggle="modal" id="blog_description" data-target="#modal-blog"  value="{{$item->id}}" data-blog_description={{ $item->description }} >-->
          <!--    View Description-->
          <!--</button></td>-->
          <td></td>
          ...
          <td><img src="{{ asset('storage/blogs/'.$item->image) }}" height="50px" width="50px" alt="" title=""></td>
          <td>  
            <div class="row"> 
              @if(auth()->user()->permission('edit blogs')) 
             <a href="{{ URL::to('blogs/' . $item->id . '/edit')}}" class="">
...
<script>
  $('#blog_description').click(function(e) {
        e.preventDefault();
         $('#body_value').html('');
      //   $('#d_value').html($(this).data('blog_description'));
        $('#blog_description').val($(this).val());
        
      let _url = `{!! url('/view_blog_desc/') !!}/` + $('#blog_description').val();
      $.ajax({
          url: _url,
          type: 'GET',
